...
public class SpellFrostCone extends ContinousSpellItem {

    public SpellFrostCone() {super(new Properties(), SpellDomain.PRIMAL, SpellMethod.FORCE, SpellAspect.FROST,1, 1f, 2);}
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            if (i % 2 == 0 && ticks % 2 == 0) {
                ...
                if (level.isClientSide()) {
                    level.addParticle(ModParticleTypes.SNOWFLAKE.get(), player.getX() + xdir * dist*0.5 + (random.nextFloat() - 0.5) * spread * 2,
            ...
            for (Entity entityiterator : sortedEntities) {
                if (entityiterator instanceof LivingEntity) {
                    if (entityiterator != player) {
                        ...
                        int frostcap = (int) (40 * spellpower * 20 * pierce + 100);
                        if (entityiterator.getTicksFrozen() < frostcap) {
                            entityiterator.setTicksFrozen(entityiterator.getTicksFrozen() + (int) (40 * spellpower * pierce + 25));
                        }
                        pierce = pierce * 0.8;
                    }
                }
